...
By default, this theme includes files from 3 CDNs (Bootstrap-CDN, Google-CDN
and Maxcdn). All those files (i.e. scripts, styles and fonts) are retrieved via
https, such that a `MITM
<https://en.wikipedia.org/wiki/Man-in-the-middle_attack>` person can't mess
with them. To increase security, against a compromised CDN, the integrity of
those files is protected via cryptographic hashes that are verified by the
browser (cf. `Subresource Integritiy - SRI
<https://en.wikipedia.org/wiki/Subresource_Integrity>`). That means that the
references generated by this theme include SRI information.

In addition, the used CDNs must be part of `Content-Security-Policy (CSP)
<https://en.wikipedia.org/wiki/Content_Security_Policy>` header - if one is
configured - which usually is a good idea.

For example, a policy in Nginx configuration syntax that covers the external
resources used by this theme::
